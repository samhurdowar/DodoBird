...
namespace SourceControl.FormLayoutClasses
{
	public partial class FormLayout
	{
		public string NumericTextbox(ColumnDef columnDef)
		{
			...
			sb.AppendLine("<input id='P" + PageTemplateId + "_" + columnDef.ColumnName + "' name='P" + PageTemplateId + "_" + columnDef.ColumnName + "'" + ((columnDef.NumberMin > 0) ? " min='" + columnDef.NumberMin + "'" : "") + " " + ((columnDef.NumberMax > 0) ? " max='" + columnDef.NumberMax + "'" : "") + "   style='width:" + columnWidth + "px;' />");
